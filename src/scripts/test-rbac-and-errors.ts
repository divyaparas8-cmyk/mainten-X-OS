...
async function testRbacAndErrors() {
  ...
  for (const r of roles) {
    try {
      const res = await (globalThis as any).fetch(`${BASE_URL}/auth/login`, {
        method: "POST",
  ...
  try {
    ...
    const res = await (globalThis as any).fetch(`${BASE_URL}/production/orders`, {
      method: "POST",
      headers: {
  ...
  try {
    const res = await (globalThis as any).fetch(`${BASE_URL}/master-data/routings/not-a-valid-uuid`, {
  ...
  try {
    ...
    const res = await (globalThis as any).fetch(`${BASE_URL}/traceability/genealogy/${randomUuid}`, {
      headers: tokens["System Admin"] ? { Authorization: `Bearer ${tokens["System Admin"]}` } : {},
    });
    const data = await res.json();
  ...
  try {
    ...
    const badPinRes = await (globalThis as any).fetch(`${BASE_URL}/auth/sign-off`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
    ...
    // Test with valid PIN
    const goodPinRes = await (globalThis as any).fetch(`${BASE_URL}/auth/sign-off`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        ...(qualityToken ? { Authorization: `Bearer ${qualityToken}` } : {}),
