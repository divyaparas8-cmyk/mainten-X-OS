...
async function runExecutiveDashboardTests() {
  ...
  await executiveService.ensureExecutiveDataSeeded(tenantId);
  const dbPlants = await db.select().from(plants).where(eq(plants.tenantId, tenantId));
  ...
  const summary = await executiveService.getDashboardSummary(tenantId, "ALL");
  ...
  if (primaryPlantId) {
    const plantSummary = await executiveService.getDashboardSummary(tenantId, primaryPlantId);
