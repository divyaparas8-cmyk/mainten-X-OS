...
export class PlanningService {
  ...
  async deleteCustomerOrder(tenantId: string, id: string) {
    const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id);
    if (isUuid) {
      await db.update(customerOrders).set({ status: "Cancelled" }).where(and(eq(customerOrders.tenantId, tenantId), eq(customerOrders.id, id)));
    } else {
      await db.update(customerOrders).set({ status: "Cancelled" }).where(and(eq(customerOrders.tenantId, tenantId), eq(customerOrders.orderNumber, id)));
    }
  ...
  async updateForecast(tenantId: string, id: string, input: UpdateForecastInput) {
    ...
    if (isUuid && Object.keys(updateValues).length > 0) {
      await db.update(forecasts).set(updateValues).where(and(eq(forecasts.tenantId, tenantId), eq(forecasts.id, id)));
    }
