...
async function runE2ETests() {
  ...
  await (adminService as any).ensurePermissionsSeeded();
  ...
  if (!updatedPerms["BOM / Recipe"]) updatedPerms["BOM / Recipe"] = {};
  ...
  // Verify DB state
  const [pmRole] = await db.select().from(roles).where(eq(roles.code, "plant_manager")).limit(1);
  const [bomDeletePerm] = await db
    .select()
    .from(permissions)
    .where(and(eq(permissions.module, "BOM / Recipe"), eq(permissions.action, "delete")))
    .limit(1);

  const [revokedRow] = await db
    .select()
    .from(rolePermissions)
    .where(and(eq(rolePermissions.roleId, pmRole.id), eq(rolePermissions.permissionId, bomDeletePerm.id)))
    .limit(1);
  ...
  const testDenied = await adminService.testPermissionAccess({
    roleKey: "plant_manager",
    module: "BOM / Recipe",
    action: "delete"
  });
  ...
  const testGranted = await adminService.testPermissionAccess({
    roleKey: "plant_manager",
    module: "BOM / Recipe",
    action: "delete"
  });
  ...
  const opTest = await adminService.testPermissionAccess({
    roleKey: "operator",
    module: "Production",
    action: "delete"
  });
  ...
  const adminTest = await adminService.testPermissionAccess({
    roleKey: "admin",
    module: "Audit Trail",
    action: "approve"
  });
