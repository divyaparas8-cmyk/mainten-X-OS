...
async function api(endpoint: string, method: string, token: string, body?: any): Promise<{ status: number; data: any }> {
...
async function runValidation() {
  ...
  const [dbSku] = await db.select().from(skus).where(eq(skus.skuCode, skuCode));
  results.push({
    module: "Master Data",
    action: "Create SKU",
  ...
  const [dbWc] = await db.select().from(workCenters).where(eq(workCenters.code, wcCode));
  results.push({
    module: "Master Data",
  ...
  const [dbLine] = await db.select().from(productionLines).where(eq(productionLines.code, lineCode));
  results.push({
    module: "Master Data",
  ...
  const [dbRtg] = await db.select().from(routings).where(eq(routings.routingCode, rtgCode));
  const rtgStepsList = dbRtg ? await db.select().from(routingSteps).where(eq(routingSteps.routingId, dbRtg.id)) : [];
  results.push({
    module: "Master Data",
  ...
  const [dbOrder] = await db.select().from(customerOrders).where(eq(customerOrders.orderNumber, orderNum));
  results.push({
    module: "Planning",
  ...
  const [dbFc] = await db.select().from(forecasts).where(eq(forecasts.skuId, dbSku?.id || "")).orderBy(desc(forecasts.createdAt)).limit(1);
  results.push({
    module: "Planning",
  ...
  const [dbAps] = await db.select().from(apsSchedules).where(eq(apsSchedules.skuId, dbSku?.id || "")).orderBy(desc(apsSchedules.createdAt)).limit(1);
  results.push({
    module: "Planning",
  ...
  const [dbProdOrder] = await db.select().from(productionOrders).where(eq(productionOrders.id, createdOrder?.id || ""));
  results.push({
    module: "Production",
  ...
  const [dbProdOrderUpdated] = await db.select().from(productionOrders).where(eq(productionOrders.id, dbProdOrder?.id || ""));
  results.push({
    module: "Production",
  ...
  const [dbProdOrderComplete] = await db.select().from(productionOrders).where(eq(productionOrders.id, dbProdOrder?.id || ""));
  results.push({
    module: "Production",
  ...
  const [activeBatch] = await db.select().from(batches).where(eq(batches.productionOrderId, dbProdOrder?.id || "")).limit(1);
  ...
  const [dbCcpPass] = await db.select().from(ccpChecks).where(eq(ccpChecks.ccpCode, "CCP-1")).orderBy(desc(ccpChecks.checkedAt)).limit(1);
  results.push({
    module: "Quality",
  ...
  const [dbCcpFail] = await db.select().from(ccpChecks).where(eq(ccpChecks.status, "FAIL")).orderBy(desc(ccpChecks.checkedAt)).limit(1);
  results.push({
    module: "Quality",
  ...
  const [dbHold] = await db.select().from(qualityHolds).where(eq(qualityHolds.lotNumber, holdLotCode));
  results.push({
    module: "Quality",
  ...
  const [dbRelease] = await db.select().from(qaReleases).where(eq(qaReleases.batchId, activeBatch?.id || "")).orderBy(desc(qaReleases.releasedAt)).limit(1);
  results.push({
    module: "Quality",
  ...
  const [firstLot] = await db.select().from(inventoryLots).where(eq(inventoryLots.tenantId, tenant.id)).limit(1);
  ...
  const [dbReceiptTx] = await db.select().from(inventoryTransactions).where(eq(inventoryTransactions.referenceId, receiptRef));
  results.push({
    module: "Warehouse",
  ...
  const [dbTransferTx] = await db.select().from(inventoryTransactions).where(eq(inventoryTransactions.referenceId, transferRef));
  results.push({
    module: "Warehouse",
  ...
  const [firstAsset] = await db.select().from(assets).where(eq(assets.tenantId, tenant.id)).limit(1);
  ...
  const [dbWO] = await db.select().from(workOrders).where(eq(workOrders.id, createdWO?.id || ""));
  results.push({
    module: "Maintenance",
  ...
  const [dbWOProg] = await db.select().from(workOrders).where(eq(workOrders.id, dbWO?.id || ""));
  results.push({
    module: "Maintenance",
  ...
  const [dbWOComp] = await db.select().from(workOrders).where(eq(workOrders.id, dbWO?.id || ""));
  results.push({
    module: "Maintenance",
  ...
  const [dbRecall] = await db.select().from(recallEvents).where(eq(recallEvents.targetLotNumber, recallTargetLot)).orderBy(desc(recallEvents.createdAt)).limit(1);
  results.push({
    module: "Traceability",
