...
export class PlanningController {
  async getCustomerOrders(request: FastifyRequest, reply: FastifyReply) {
    const data = await planningService.listCustomerOrders(request.user.tenantId, request.user.plantId);
  ...
  async createCustomerOrder(request: FastifyRequest, reply: FastifyReply) {
    ...
    const data = await planningService.createCustomerOrder(request.user.tenantId, request.user.plantId || "default-plant", input);
  ...
  async runForecast(request: FastifyRequest, reply: FastifyReply) {
    ...
    const data = await planningService.runStatisticalForecast(request.user.tenantId, request.user.plantId || "default-plant", input);
  ...
  async getApsSchedules(request: FastifyRequest, reply: FastifyReply) {
    const data = await planningService.listApsSchedules(request.user.tenantId, request.user.plantId);
    return reply.send(formatSuccess(data));
  }

  async createApsSchedule(request: FastifyRequest, reply: FastifyReply) {
    const input = createApsScheduleSchema.parse(request.body);
    const data = await planningService.createApsSchedule(request.user.tenantId, request.user.plantId || "default-plant", input);
  ...
  async getMrpExplosion(request: FastifyRequest, reply: FastifyReply) {
    const data = await planningService.runMrpExplosion(request.user.tenantId, request.user.plantId || "default-plant");
