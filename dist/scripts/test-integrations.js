...
const razorpay_adapter_js_1 = require("../modules/billing/razorpay.adapter.js");
const machineData_service_js_1 = require("../modules/iot/services/machineData.service.js");
...
async function runIntegrationVerification() {
    ...
    try {
        ...
        const res = await ai_service_js_1.aiService.chatQuery("What is the current OEE and downtime on packaging lines?");
        ...
        const vibRes = await ai_service_js_1.aiService.chatQuery("Check bearing vibration for capper spindle");
        ...
        try {
            await ai_service_js_1.aiService.chatQuery("   ");
        }
        catch {
            threwEmpty = true;
        }
    ...
    try {
        ...
        const order = await billing_service_js_1.billingService.createOrder({ planId: "standard", tenantId: testTenantId, currency: "INR" });
        ...
        const fakeOrderId = order.orderId;
        const fakePaymentId = `pay_${Date.now().toString(36)}`;
        const secret = process.env.RAZORPAY_KEY_SECRET || "placeholder_secret_key_2026";
        const validSignature = crypto_1.default.createHmac("sha256", secret).update(`${fakeOrderId}|${fakePaymentId}`).digest("hex");
        const isSigValid = razorpay_adapter_js_1.razorpayAdapter.verifyPaymentSignature(fakeOrderId, fakePaymentId, validSignature);
        ...
        const isTamperedRejected = !razorpay_adapter_js_1.razorpayAdapter.verifyPaymentSignature(fakeOrderId, fakePaymentId, tamperedSig);
        ...
        const verifyRes = await billing_service_js_1.billingService.verifyPayment({
        ...
        const currentSub = await billing_service_js_1.billingService.getSubscription(testTenantId);
        ...
        const webhookSig = crypto_1.default.createHmac("sha256", webhookSecret).update(webhookRawBody).digest("hex");
        // First delivery
        const whRes1 = await billing_service_js_1.billingService.processWebhook(webhookRawBody, webhookSig, webhookPayload);
        ...
        const whRes2 = await billing_service_js_1.billingService.processWebhook(webhookRawBody, webhookSig, webhookPayload);
    ...
    try {
        const adapters = machineData_service_js_1.machineDataService.getAdapters();
        // Test 3.1: Ingestion via Synthetic Simulator
        const simEvent = await machineData_service_js_1.machineDataService.ingest({
            source: "SIMULATOR",
            assetId: "FM-001",
            assetCode: "FM-001",
            speed: 252.5,
        ...
        const latestOpc = machineData_service_js_1.machineDataService.getLatestTelemetry("FM-001");
        ...
        const latestMqtt = machineData_service_js_1.machineDataService.getLatestTelemetry("FM-001");
        ...
        const latestModbus = machineData_service_js_1.machineDataService.getLatestTelemetry("CP-002");
        ...
        const alarmEvent = await machineData_service_js_1.machineDataService.ingest({
            source: "SIMULATOR",
            assetId: "FM-001",
            assetCode: "FM-001",
            speed: 240,
        ...
        const history = await machineData_service_js_1.machineDataService.getHistory("FM-001", 5);
    ...
    await machineData_service_js_1.machineDataService.stopSimulator();
