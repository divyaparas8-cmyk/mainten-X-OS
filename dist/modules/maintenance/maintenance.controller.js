...
class MaintenanceController {
    async getWorkOrders(request, reply) {
        const data = await maintenance_service_js_1.maintenanceService.listWorkOrders(request.user.tenantId, request.user.plantId);
    ...
    async createWorkOrder(request, reply) {
        ...
        const data = await maintenance_service_js_1.maintenanceService.createWorkOrder(request.user.tenantId, request.user.plantId || "default-plant", input, request.user.userId);
