...
class WarehouseController {
    async getLots(request, reply) {
        const data = await warehouse_service_js_1.warehouseService.listLots(request.user.tenantId, request.user.plantId);
        return reply.send((0, responseFormatter_js_1.formatSuccess)(data));
    }
    async createLot(request, reply) {
        const input = warehouse_schema_js_1.createLotSchema.parse(request.body);
        const data = await warehouse_service_js_1.warehouseService.createLot(request.user.tenantId, request.user.plantId || "default-plant", input);
    ...
    async recordTransaction(request, reply) {
        ...
        const data = await warehouse_service_js_1.warehouseService.recordTransaction(request.user.tenantId, request.user.plantId || "default-plant", input, request.user.userId);
