...
async function runMasterAdminTestSuite() {
    ...
    try {
        ...
        const defaultHash = await bcryptjs_1.default.hash("Password@123", 10);
        await database_js_1.pool.query("UPDATE users SET password_hash = $1 WHERE email = 'admin@maintenx.com'", [defaultHash]);
        ...
        assert(matchingCompanies.some((c) => c.id === testCompanyId), "Newly Created Company Found in Search Results");
        ...
        assert(auditLogsData.some((l) => l.event.includes("COMPANY") || l.event.includes("PLAN") || l.event.includes("MODULE")), "Mutations from Earlier Tests Recorded Real Audit Entries in PostgreSQL");
        // --------------------------------------------------------------------------
