...
class MasterAdminService {
    ...
    async createCompany(input, actor) {
        ...
        try {
         ...
         ON CONFLICT (email) DO UPDATE SET
           ...
           status = 'ACTIVE'
         RETURNING *`, [newTenant.id, input.adminEmail.toLowerCase().trim(), defaultPasswordHash, firstName, lastName, input.adminPhone || null, defaultPinHash]);
            const newUser = userRows[0];
            // 4. Assign Admin Role
            const { rows: roleRows } = await client.query(`SELECT id FROM roles WHERE code = 'admin' LIMIT 1`);
            if (roleRows.length > 0) {
                await client.query(`INSERT INTO user_roles ("userId", "roleId") VALUES ($1, $2) ON CONFLICT DO NOTHING`, [newUser.id, roleRows[0].id]);
            }
    ...
    async createCompanyAdmin(input, actor) {
        ...
        try {
         ...
         RETURNING *`, [tenant.id, email, defaultPasswordHash, firstName, lastName, defaultPinHash]);
            const newUser = userRows[0];
            const { rows: roleRows } = await client.query(`SELECT id FROM roles WHERE code = 'admin' LIMIT 1`);
            if (roleRows.length > 0) {
                await client.query(`INSERT INTO user_roles ("userId", "roleId") VALUES ($1, $2) ON CONFLICT DO NOTHING`, [newUser.id, roleRows[0].id]);
    ...
    async deletePlan(id, actor) {
        ...
        const usedInSubs = await database_js_1.db.select().from(index_js_1.subscriptions).where((0, drizzle_orm_1.eq)(index_js_1.subscriptions.planId, id)).limit(1);
        if (usedInSubs.length > 0) {
            // Soft-deactivate instead of breaking foreign keys
            await database_js_1.db.update(index_js_1.plans).set({ status: "Inactive", updatedAt: new Date() }).where((0, drizzle_orm_1.eq)(index_js_1.plans.id, id));
